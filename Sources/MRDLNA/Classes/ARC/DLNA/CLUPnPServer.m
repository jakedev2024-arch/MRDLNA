...
- (void)udpSocket:(GCDAsyncUdpSocket *)sock didSendDataWithTag:(long)tag{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(weakSelf.searchTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (weakSelf.receiveDevice && weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(upnpSearchChangeWithResults:)]){
            [weakSelf.delegate upnpSearchChangeWithResults:weakSelf.deviceDictionary.allValues];
        }
